...
int initTokenizer(int argc, char *argv[]){
    char *path;
...
Token* checkIdentifier(char token[]){
    ...
    if(isalpha(c) || c == '_' || c == '$'){
        ...
        //checks if token is cin:
        if(strcmp(token, "cin") == 0){
            return tokenInit(token, CIN);
        }

        //checks if token is cout:
        if(strcmp(token, "cout") == 0){
            return tokenInit(token, COUT);
        }
...
Token* checkKeyword(char token[]){
    ...
    Type type = getTokenTypeFromString(potentialKw);
...
Token* checkPunctuators(char token[]){
    ...
    switch(c){
        ...
        case '}':
            return tokenInit(token, CLOSE_BRACE);
